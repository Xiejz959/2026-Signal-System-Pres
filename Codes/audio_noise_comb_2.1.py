...
def synth_clean_voice() -> list[float]:
    # Build a longer "announcement-like" sequence so the demo audio is long enough
    # for playback, clipping, and visualization in class.
    phrase_templates = [
        [
            (0.42, 135, 145, [(730, 90), (1090, 120), (2440, 180)]),
            (0.06, 0, 0, []),
            (0.34, 142, 155, [(530, 80), (1840, 130), (2480, 170)]),
            (0.05, 0, 0, []),
            (0.30, 148, 160, [(270, 70), (2290, 130), (3010, 200)]),
            (0.07, 0, 0, []),
            (0.42, 140, 132, [(570, 90), (840, 100), (2410, 190)]),
            (0.06, 0, 0, []),
            (0.40, 130, 126, [(300, 70), (870, 100), (2240, 170)]),
        ],
        [
            (0.36, 145, 150, [(530, 80), (1840, 130), (2480, 170)]),
            (0.06, 0, 0, []),
            (0.46, 132, 138, [(730, 90), (1090, 120), (2440, 180)]),
            (0.05, 0, 0, []),
            (0.34, 150, 158, [(270, 70), (2290, 130), (3010, 200)]),
            (0.07, 0, 0, []),
            (0.44, 138, 130, [(570, 90), (840, 100), (2410, 190)]),
            (0.09, 0, 0, []),
            (0.40, 128, 124, [(300, 70), (870, 100), (2240, 170)]),
        ],
        [
            (0.40, 136, 146, [(730, 90), (1090, 120), (2440, 180)]),
            (0.05, 0, 0, []),
            (0.32, 144, 152, [(530, 80), (1840, 130), (2480, 170)]),
            (0.05, 0, 0, []),
            (0.36, 152, 160, [(270, 70), (2290, 130), (3010, 200)]),
            (0.06, 0, 0, []),
            (0.48, 138, 132, [(570, 90), (840, 100), (2410, 190)]),
            (0.08, 0, 0, []),
            (0.44, 132, 128, [(300, 70), (870, 100), (2240, 170)]),
        ],
        [
            (0.46, 134, 140, [(730, 90), (1090, 120), (2440, 180)]),
            (0.06, 0, 0, []),
            (0.38, 146, 154, [(530, 80), (1840, 130), (2480, 170)]),
            (0.05, 0, 0, []),
            (0.34, 150, 156, [(270, 70), (2290, 130), (3010, 200)]),
            (0.07, 0, 0, []),
            (0.50, 140, 134, [(570, 90), (840, 100), (2410, 190)]),
            (0.08, 0, 0, []),
            (0.46, 130, 126, [(300, 70), (870, 100), (2240, 170)]),
        ],
    ]

    out: list[float] = []
    for idx, segments in enumerate(phrase_templates):
        for duration, f0a, f0b, formants in segments:
            if not formants:
                out.extend(silence(duration))
            else:
                out.extend(synth_vowel(duration, f0a, f0b, formants))
        # Pause between "announcement phrases".
        if idx < len(phrase_templates) - 1:
            out.extend(silence(0.6))
...
def read_audio(path: str) -> tuple[list[float], int]:
    """Read audio file and return samples and sampling rate."""
    data, fs = sf.read(path)
    return data.tolist(), fs


def save_audio(path: str, samples: list[float], fs: int) -> None:
...
def main() -> None:
    # 读取音频文件
    clean, fs_clean = librosa.load("D:/Courses/2026Spring/SigNSys/Codes/generated_audio/clean_voice.mp3", sr=None)
    noise, fs_noise = librosa.load("D:/Courses/2026Spring/SigNSys/Codes/generated_audio/hiss_noise.mp3", sr=None)

    # 对齐音频长度
    if len(clean) > len(noise):
        # 延长噪声长度
        repeat_count = len(clean) // len(noise)
        remainder = len(clean) % len(noise)
        noise = np.tile(noise, repeat_count)
        noise = np.concatenate((noise, noise[:remainder]))
    ...
    # 保存混合后的音频到指定路径
    save_audio(Path("D:/Courses/2026Spring/SigNSys/Codes/generated_audio/noisy_voice.mp3"), noisy, fs_clean)

    print("Generated noisy_voice.mp3 successfully.")
